...
int main (void) {
	...
	file_zeo = "data/atom_sites/" + name_zeo + ".txt";
	file_ucell = "data/unit_cell/" + name_zeo + ".txt";
...
vector<int> merw_substitutions (int **M_T, int Nsubst) {
	...
	for(int i=1; i<Nsubst; i++) {
		...
		//Select randomly the initial position of the MERW
...
int merw_step(double **S, vector<vector<int> > neigbrs, int i) {
	...
	for(int j=0; j<Noptions && ctl==0; j++) {
		acc+=probs[j];
		if(p <= acc) {
			next=neigbrs[i][j];
			ctl=1;
...
void print_structure (atom *list, vector<int> Als, int struc, string name_zeo, string name_alg, string out_name) {
	...
	fout << "_cell_angle_alpha 90" << endl;
	fout << "_cell_angle_beta  90" << endl;
	fout << "_cell_angle_gamma 90" << endl;
	fout << "_cell_volume      " << a*b*c << endl << endl;

	fout << "_symmetry_cell_setting          orthorhombic" << endl;
